...
void Debug::Write(const std::u8string_view& message)
{
    ...
    const auto utf16MessageBytes = Encoding::Convert(Encoding::UTF8(), Encoding::Unicode(), reinterpret_cast<const std::byte*>(message.data()), static_cast<int32_t>(message.size()), reinterpret_cast<std::byte*>(&g_tempUtf16Buffer[0]), static_cast<int32_t>(g_tempUtf16Buffer.size()));
    if (utf16MessageBytes == -1)
    {
        return;
    }

    const int32_t utf16MessageLen = utf16MessageBytes / 2;
