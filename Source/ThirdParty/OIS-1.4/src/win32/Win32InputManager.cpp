...
BOOL CALLBACK Win32InputManager::_DIEnumDevCallback(LPCDIDEVICEINSTANCE lpddi, LPVOID pvRef)
{
		...
		GET_DIDEVICE_TYPE(lpddi->dwDevType) == DI8DEVTYPE_SUPPLEMENTAL)
	{
		JoyStickInfo jsInfo;
        {
            ...
            size_t vendorStrLen = wcslen(lpddi->tszInstanceName);
            jsInfo.vendor.resize(vendorStrLen + 1);
            wcstombs_s(nullptr, &jsInfo.vendor[0], vendorStrLen + 1, lpddi->tszInstanceName, vendorStrLen + 1);
